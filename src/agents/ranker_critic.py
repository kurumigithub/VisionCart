...
1. **Critic stage** — filters obvious mismatches (material / colour /
   constraint violations, very low image similarity).
2. **Ranker stage** — scores remaining products on three dimensions
   (image similarity, text similarity, semantic match) and returns a
   ranked list.

Scoring components
------------------
* **image_similarity** — cosine similarity between ``board_embedding``
  and each product's ``image_embedding``.
* **text_similarity** — keyword-overlap between the style summary /
  keywords and the product's combined text fields.
* **semantic_match_score** — rule-based score that rewards preferred
  materials / colours and penalises avoided ones.

The final score is a configurable weighted sum::

    final = w_img * image + w_txt * text + w_sem * semantic

Thresholds and weights live in ``src/config.py`` and can also be
overridden at call time via the ``config`` key in state.

Splitting Critic / Ranker later
-------------------------------
The two logical stages are already isolated into ``_critic_evaluate``
and ``_rank_products``.  To split them into separate agents, move each
function into its own module and wire them as separate LangGraph nodes.
"""
...
def _semantic_match_score(
    ...
    *,
    ...
    Starts at 0.5 (neutral), gets bonuses for preferred matches and
    penalties for avoid-list violations.
    """
    score = 0.5
    ...
    # Preferred-material / colour bonuses
    ...
    for term in _list_lower(color_palette.get("accent", [])):
        if term in product_text:
            score += preferred_bonus * 0.5  # accent colours worth less

    # Keyword / style-element bonuses
    all_keywords = style_keywords + style_elements
    if all_keywords:
        kw_hit_rate = keyword_overlap_score(all_keywords, product_text)
        score += kw_hit_rate * 0.2  # up to +0.2
...
def _critic_evaluate(
    ...
    *,
    ...
    """
    Return a rejection reason string, or ``None`` if the product passes.

    When *has_image_scores* is False (missing embedding on either side),
    the image-similarity floor check is skipped and the overall score is
    computed with rebalanced weights so the product is judged on the
    dimensions that *are* available.
    """
    # 1. Must-avoid hard constraint
    ...
    # 2. Image similarity floor — skip when embeddings are unavailable
    if has_image_scores and image_sim < img_reject_floor:
        return (
            f"Rejected: image similarity too low "
            f"({image_sim:.2f} < {img_reject_floor:.2f})."
        )

    # 3. Avoid-list violations severe enough to reject
    color_palette = style_profile.get("color_palette") or {}
    materials_info = style_profile.get("materials") or {}
    violations = _check_avoid_lists(product, color_palette, materials_info)
    if len(violations) >= 2:
        return (
            f"Rejected: multiple avoid-list violations "
            f"({', '.join(violations)})."
        )

    # 4. Overall score check (rebalance weights when image is missing)
    if has_image_scores:
        final = w_img * image_sim + w_txt * text_sim + w_sem * semantic_score
    else:
        # Redistribute image weight across text and semantic proportionally
        non_img = w_txt + w_sem
        if non_img > 0:
    ...
    if final < overall_reject_threshold:
        return (
            f"Rejected: overall score too low "
            f"({final:.2f} < {overall_reject_threshold:.2f})."
        )
...
def _compute_scores(
    ...
    *,
    ...
    # Combine style keywords with words from the summary
    combined_keywords = list(style_keywords)
    if style_summary:
        # Add meaningful words from summary (length > 3 to skip articles)
        combined_keywords.extend(
            w for w in normalize_text(style_summary).split()
            if len(w) > 3
        )
...
def _build_reason(score: ScoreBreakdown, final: float) -> str:
    """Generate a short human-readable reason for accepting a product."""
    parts: List[str] = []
...
def rank_and_critique(
    ...
    """
    Core logic — can be called directly (for testing) or via ``run()``.

    Parameters
    ----------
    style_profile : dict
        Style profile JSON from the Stylist agent.
    candidate_products : list[dict]
        Candidate products from the Procurement agent.
    config : dict, optional
        Override any threshold / weight at call time.
    ...
    must_avoid_reject = bool(
        (config or {}).get("must_avoid_auto_reject", MUST_AVOID_AUTO_REJECT)
    )
    ...
    for product in candidate_products:
        ...
        rejection_reason = _critic_evaluate(
            product,
            style_profile,
            scores.image_similarity,
            scores.semantic_match_score,
            img_reject_floor=img_reject_floor,
            overall_reject_threshold=overall_reject,
            must_avoid_auto_reject=must_avoid_reject,
            w_img=w_img,
            w_txt=w_txt,
            w_sem=w_sem,
            text_sim=scores.text_similarity,
            has_image_scores=scoring.has_image_scores,
        )
        if rejection_reason:
            rejected.append(RejectedProduct(product_id=pid, reason=rejection_reason))
            continue

        # Compute final score (rebalance when image is missing)
        if scoring.has_image_scores:
            final = (
                w_img * scores.image_similarity
                + w_txt * scores.text_similarity
                + w_sem * scores.semantic_match_score
            )
        else:
            non_img = w_txt + w_sem
            if non_img > 0:
                final = (
                    (w_txt / non_img) * scores.text_similarity
                    + (w_sem / non_img) * scores.semantic_match_score
                )
            else:
                final = 0.0
        final = round(final, 4)
...
def run(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Reads from state
    ----------------
    - ``style_profile`` (dict): output from the Stylist agent.
    - ``candidate_products`` (list[dict]): output from the Procurement agent.
    - ``config`` (dict, optional): threshold / weight overrides.

    Writes to state
    ---------------
    - ``ranked_products`` (list[dict]): accepted products sorted by score.
    - ``rejected_products`` (list[dict]): products filtered out by the critic.
    - ``ranker_critic_output`` (dict): full structured output.
    """
    ...
    # Flatten accepted products into the shape the output agent expects
